...
def move(file, folder, package, report):
	if file.needToBeMovedTo(folder):
		if package.isDuplicated(file):
			if report:
				print(fmt("@{yf}[WARN] The %s file is duplicated in the %s package." % (file.name, package.name)))
		package.move(file, file.folder, folder)
		if report:
			print(fmt("[INFO] The %s file was moved to %s directory." % (file.name, folder.path)))
		...
		if not os.path.exists(folder_path):
			...
			if report:
				if os.path.exists(folder_path):
					print(fmt("[INFO] The %s folder was created at %s." % (folder_name, folder_path)))
				else:
					print(fmt("@!@{rf}[ERROR]@|@{rf} The %s folder could not be created at %s." % (folder_name, folder_path)))
					sys.exit(2)
...
def organize(package, report):
	if report:
		print(fmt("@!@{gf}==>@|@! Processing %s package..." % package.name))
	for extension, folder_name in package.extensions.items():
		files = locate(package, extension, package.path)
		if not files:
			if report:
				print(fmt("@{yf}[WARN] None %s file was found in the %s package." % (extension.upper(), package.name)))
		else:
			if not package.files_to_be_moved:
				if report:
					print(fmt("[INFO] All %s file is located at their proper folder (%s) in the %s package." % (extension.upper(), folder_name, package.name)))
			else:
...
if __name__ == "__main__":
	...
	if args.all or len(args.extensions) == 0:
		...
		expression.extend(['-e', 'config', 'machine', 'rviz', 'urdf', 'xacro', 'yaml', '-f', 'config'])
	...
	for package_name in args.packages:
		try:
			...
			if report: 
				print(fmt("@{rf}[ERROR] %s is not a ROS package." % package_name))
			continue
		for i in range(len(args.extensions)):
			...
			for folder_extension in args.extensions[i]:
				if folder_extension.startswith("."):
					if report:
						print(fmt("@{rf}[ERROR] The extension names must be entered with not dot."))
					continue
	...
	if not packages:
		if report: 
			print(fmt("@{rf}[ERROR] None valid ROS package."))
		sys.exit(2)
	...
	if report:
		...
		for package in packages:
			print(fmt("%s" % package.pretty()))
		print(fmt("End of report.\n"))